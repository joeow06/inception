#!/bin/bash
set -e

WP_PATH="/var/www/html"

# Resolve _FILE secret before anything that needs the password
if [ -n "$WORDPRESS_DB_PASSWORD_FILE" ] && [ -f "$WORDPRESS_DB_PASSWORD_FILE" ]; then
    WORDPRESS_DB_PASSWORD=$(cat "$WORDPRESS_DB_PASSWORD_FILE")
    export WORDPRESS_DB_PASSWORD
fi

# Wait for MariaDB to be reachable
echo "Waiting for MariaDB..."
until mariadb -h"${WORDPRESS_DB_HOST}" -u"${WORDPRESS_DB_USER}" -p"${WORDPRESS_DB_PASSWORD}" "${WORDPRESS_DB_NAME}" -e "SELECT 1;" >/dev/null 2>&1; do
    echo "MariaDB not ready, retrying..."
    sleep 2
done
echo "MariaDB is ready!"

echo "Setting up WordPress..."

# Download WordPress core files if missing
if [ ! -f "$WP_PATH/wp-login.php" ]; then
    echo "Downloading WordPress..."
    wget -q https://wordpress.org/latest.tar.gz -O /tmp/wordpress.tar.gz
    tar -xzf /tmp/wordpress.tar.gz -C /tmp
    rm /tmp/wordpress.tar.gz
    cp -rn /tmp/wordpress/* "$WP_PATH" || true
    rm -rf /tmp/wordpress
fi

# Always regenerate wp-config.php so secrets are always current
echo "Writing wp-config.php..."
WP_SALTS=$(wget -qO- https://api.wordpress.org/secret-key/1.1/salt/)
cat > "$WP_PATH/wp-config.php" << EOF
<?php
define('DB_NAME', '${WORDPRESS_DB_NAME}');
define('DB_USER', '${WORDPRESS_DB_USER}');
define('DB_PASSWORD', '${WORDPRESS_DB_PASSWORD}');
define('DB_HOST', '${WORDPRESS_DB_HOST}');
define('DB_CHARSET', 'utf8');
define('DB_COLLATE', '');
\$table_prefix = '${WORDPRESS_TABLE_PREFIX:-wp_}';
${WP_SALTS}
define('WP_DEBUG', false);
if ( !defined('ABSPATH') )
    define('ABSPATH', __DIR__ . '/');
require_once ABSPATH . 'wp-settings.php';
EOF

find "$WP_PATH" -type d -exec chmod 750 {} \;
find "$WP_PATH" -type f -exec chmod 640 {} \;
chown -R www-data:www-data "$WP_PATH"

echo "WordPress setup complete."
echo "Starting PHP-FPM..."
exec php-fpm8.2 -F
